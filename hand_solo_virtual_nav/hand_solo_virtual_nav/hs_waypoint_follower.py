...
def main():
    ...
    waypoints = [
        make_pose(8.0, 14.0, 0.0),
        make_pose(30.0, 14.0, (3 * math.pi / 2)),
        make_pose(30.0, 3.0, math.pi),
        make_pose(7.0, 0.0, math.pi),
        make_pose(3.0, -3.0, math.pi),
        make_pose(-4.0, -2.0, math.pi / 2),
        make_pose(0.0, 0.0, 0.0),
    ]
    
    for idx, wp in enumerate(waypoints, start=2):
        send_and_wait(wp)
        node.get_logger().info(f'Waiting {wait_seconds:.0f} seconds at waypoint {idx}...')
